...
ClassChild::ClassChild(const char* name, list<AstNode*> *class_members)
{
  name1=name;
//  if(class_members==NULL)
//    class_members1=CLASS_CHILD;
  class_members1 = class_members;
}


MethodChild::MethodChild(const char* name, Type *return_type,list <AstNode*> *formal_params,Stmt *method_body)
...
FieldChild::FieldChild(const char *name,Type *type,int dimensions)
{
  name1=name;
  type1 = type;
...
VariableChild::VariableChild(const char *name,
             Type *type,
             int dimensions) {
  name1=name;
  type1 = type;
...
CallOutInvocation::CallOutInvocation(StringConstant *str,list <Expr *> *expr)
{
   str1=str;
   expr1=expr;
}
...
DeclStmt::DeclStmt(list<AstNode*>* var_list){
   var_list1 = var_list;
}
...
BinaryExpr::BinaryExpr(BinaryOperator binary_operator, Expr* lhs, Expr* rhs){
...
LocationExpr::LocationExpr(Expr *id,Expr *expr)
{
   id1=id;
   expr1=expr;
}
